...
<script>
  ...
  function countup() {
    ...
    if (state.onmove == "1") {
      if (mystate != 'onmove') {
        if (state.move_to == current_location) {
          console.log('coming')
          showOnmove(state)
          return 
        } else {
  ...
  <?php else:?>
    ...
    setInterval(precountup, remotetimer);
  <?php endif;?>  
  // 移動中の時　→　移動中画面
  // 停止中の時　→　目的地一覧
  ...
  function showSelect(state) {
    ...
    Object.keys(destination_list).forEach(function(key) {
      $('.boxlist').append('<div class="selectbox" onclick="setDestinationDestination(\''+destination_list[key]+'\')" data="'+destination_list[key]+'" >'+destination_list[key]+'</div>')
    });
    string = '現在地を選択してください'
