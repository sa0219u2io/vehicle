...
//2022.12.23 suehirotei add --->>>
    // データソート (順番: "レジ"、数字以外、数字昇順･･･)
    deslist[i].destination_list.sort((a, b)=>{
      var r = 0;
      if((isNaN(a.destination_name) == true) && isNaN(b.destination_name) == true){
        // 文字列の中でも"レジ"を先頭にして、"レジ"以外は文字列昇順
        if(a.destination_name == "レジ"){r = -1;}
        else if(b.destination_name == "レジ"){r = 1}
        else if(a.destination_name < b.destination_name){r = -1;}
        else {r = 1;}
      }
      // 数字以外の文字列は先頭にする
      else if(isNaN(a.destination_name) == true){r = -1;}
      else if(isNaN(b.destination_name) == true){r = 1;}
      // 数字を昇順にする
      else if(Number(a.destination_name) < Number(b.destination_name)){ r = -1; }
      else if(Number(a.destination_name) > Number(b.destination_name)){ r = 1; }
      return r;
    });
//2022.12.23 suehirotei add <<<---
