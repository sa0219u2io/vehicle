...
function announcelist() {
  ...
  Object.keys(announce).forEach(function (key) {
    $('#modal-announce').append('<div class="flexbox" onclick="readJson(\''+key+'\',\'99\')">'+anlist[key]+'</div><hr>')
  });
}


function viewAnnounce(type, response) {
  console.log('type:'+'announce-'+type)
  openModalBG('announce-'+type);
  console.log(response)

  var announce = JSON.parse(getVariable('announce'))
  var current_announce = announce[type];
