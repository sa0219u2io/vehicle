...
const mainversion = appname + '1.2.7' + '('+debug+')'
    ...
    case '0': //通常走行モード
      if (turnaround > 0) {
        ...
        if (turnaround_count >= max) {
          ...
          setTimeout(function(){
            setPinLock()
            setTimeout(function(){req_move(getVariable('move_from'))}, 2000)
          }, 8000)
...
function resume_move() {
  ...
  setPinLock()
  setTimeout(function(){setTimereq_relocate(destid)}, 3000);
}
...
function setPinDefault() {
  req_usb_command(300,1);
  setVariable('pinstate', 0);
}

//原点だし
function setPinFree() {
  req_usb_command(400,0);
  setVariable('pinstate', 0);
}

//原点だし
function setPinLock() {
  req_usb_command(400,1);
  setVariable('pinstate', 1);
}