  ...
  <body>
    <div class="wrapper">
      ...
      <div class="main">
        ...
        <button class="select float" onclick="req_usb_command(1000再設置画面,0,0)">ステータス要求</button>
        ...
        <button class="select float"  onclick="req_usb_command(300,1)">原点復帰</button>
        <button class="select float"  onclick="req_usb_command(400,0)">解錠（昇降Down）</button>
        <button class="select float"  onclick="req_usb_command(400,1)">施錠（昇降Up）</button>
        <br>
        <div class="logscroll">
          <textarea name="usblog" class="usblog" id="usblog" disabled></textarea>
          <script type="text/javascript">
            console.log = ((logTextAreaArgument) => {
              let logTextArea = logTextAreaArgument;
              return text => logTextArea.value+=JSON.stringify(text)+'\n';
            })(document.getElementsByClassName('usblog')[0]);
          </script>
          <button onclick="transScreen('usbdebug')" class="clearusbbutton">クリア</button>
        </div>
