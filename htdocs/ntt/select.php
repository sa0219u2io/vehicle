    ...
    Object.keys(destination_list_q).forEach(function (key,value) {
      ...
      if (key == current_destination) {
        if (key == 'wp010001'||key == 'wp010002'||key == 'wp010017') {
          $('#selectcontaier').append('<div class="bigselect float" id="current_location onclick="move(\''+ key +'\')">'+ destination_list_q[key] +'</div>')
        } else {
          $('#selectcontaier').append('<div class="select float" id="current_location onclick="move(\''+ key +'\')">'+ destination_list_q[key] +'</div>')
        }
        
      } else {
        if (key == 'wp010001'||key == 'wp010002'||key == 'wp010017') {
          $('#selectcontaier').append('<div class="bigselect float" onclick="move(\''+ key +'\')">'+ destination_list_q[key] +'</div>')
        } else {
          $('#selectcontaier').append('<div class="select float" onclick="move(\''+ key +'\')">'+ destination_list_q[key] +'</div>')
        }
